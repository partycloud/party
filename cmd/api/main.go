...
func (a *API) GetServer(ctx context.Context, req *pb.GetServerRequest) (*pb.GetServerResponse, error) {
	row := a.db.QueryRowContext(
		ctx, "SELECT id, name, image, hash FROM servers where guild_id = $1 and id = $2", guildID, req.Id)

	srv := &pb.Server{
		Fileset: &pb.Fileset{},
	}
	if err := row.Scan(&srv.Id, &srv.Name, &srv.Image, &srv.Fileset.Hash); err != nil {
		return nil, err
	}
...
func (a *API) ListServers(ctx context.Context, req *pb.ListServersRequest) (*pb.ListServersResponse, error) {
	...
	rows, err := a.db.QueryContext(ctx, "SELECT id, name, image, hash FROM servers where guild_id = $1", guildID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err = rows.Scan(&srv.Id, &srv.Name, &srv.Image, &srv.Fileset.Hash); err != nil {
			return nil, err
		}
	}
...
// SetFileset updates the hash for a server
func (a *API) SetFileset(ctx context.Context, req *pb.SetFilesetRequest) (*pb.SetFilesetResponse, error) {
	row := a.db.QueryRowContext(ctx, `
    UPDATE servers SET hash=$1 WHERE guild_id=$2 AND server_id=$3 RETURNING id`, req.Hash, guildID, req.ServerId)
	var returnID string
	err := row.Scan(&returnID)
	if err != nil {
		return nil, err
